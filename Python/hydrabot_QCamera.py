...
class CV2Worker(QObject):
    ...
    def find_well(self, show_result=True):
        ...
        blurred = cv2.medianBlur(cimg, p[11])
        ...
        if p[10] == 1:
            ...
            canny = cv2.Canny(cimg, p[1], p[2])
        elif p[10] == 14:
            canny = cv2.Canny(r_sub_g, p[1], p[2])
        elif p[10] == 15:
            canny = cv2.Canny(blurred, p[1], p[2])
        if p[10] <= 12:
            if p[12] == 1:
        ...
        likely_hydra = []
        for c in contours:
            if 10000 > cv2.contourArea(c) > 451:
...
class MainWindow(QMainWindow, hydragui.Ui_MainWindow):
    ...
    def scanQR(self):
        for i in range(5):
            ...
            if self.checkBlur(threshold=100, crop=True):
                self.qr_ready = False
                self.decodeQrSignal.emit(self.rgbImg[:, :, :])
                return
    ...
    def scan_well(self):
        for i in range(5):
            ...
            while self.cv2_ready is False:
                QCoreApplication.processEvents()
            if self.checkBlur(threshold=25):
                self.cv2_ready = False
                self.cv2_pass_frame_signal.emit(self.rgbImg)
                while self.cv2_ready is False:
                    QCoreApplication.processEvents()
                self.cv2_ready = False
    ...
    def initCamera(self):
        ...
        self.cam_view.setContextMenuPolicy(Qt.CustomContextMenu)
        self.cam_view.customContextMenuRequested.connect(self.view_menu)
        self.videoSurfaceLayout.addWidget(self.cam_view)
        self.cam.setCaptureMode(QCamera.CaptureStillImage)
        self.cam_exposure = self.cam.exposure()
        self.cam_modes = []
        print(QCameraExposure.ExposureAuto)
        print('Checking Modes')
        for mode in EXPOSURE_MODES:
            self.cam_modes.append(mode)
            self.exposureModeComboBox.addItem(EXPOSURE_MODES[mode], mode)
            print('%s: %i' % (EXPOSURE_MODES[mode], mode))
        current_mode = self.cam_exposure.exposureMode()
        print(current_mode)
        self.exposureModeComboBox.setCurrentIndex(self.exposureModeComboBox.findData(current_mode, Qt.UserRole))
        self.exposureModeComboBox.currentIndexChanged.connect(self.set_camera_exposure_mode)
        # self.cam_exposure.setExposureMode(QCameraExposure.ExposureFireworks)
        self.camShutterBox.editingFinished.connect(self.set_camera_shutter_speed)
        self.cam.setViewfinder(self.cam_view)
        self.cam_error = 0
        self.cam.error.connect(self.camera_error)
        self.cam.start()
    ...
    def cleanClose(self):
        ...
        if self.cam is not None:
            print('Stopping and Unloading Camera')
            self.cam.stop()
            self.cam.unload()
